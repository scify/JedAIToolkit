 ...
 * @author Georgios
 */
public class BestPipelineForD2 extends AbstractBestConfiguration {

    public BestPipelineForD2(AbstractDuplicatePropagation dp, List<EntityProfile> pf1, List<EntityProfile> pf2) {
        super(dp, pf1, pf2);
    }
    ...
    public void applyWorkflow() {
        ...
        bf.setNumberedGridConfiguration(35);
        blocks = bf.refineBlocks(blocks);

        final AbstractMetablocking mb = new WeightedEdgePruning();
        mb.setNumberedGridConfiguration(4);
        blocks = mb.refineBlocks(blocks);

        long time2 = System.currentTimeMillis();

        final ProfileMatcher pm = new ProfileMatcher(profiles1, profiles2,
                RepresentationModel.CHARACTER_TRIGRAMS_TF_IDF, SimilarityMetric.ARCS_SIMILARITY);
        simPairs = pm.executeComparisons(blocks);

        umc = new UniqueMappingClustering();
        umc.setNumberedGridConfiguration(17);
    ...
    public static void main(String[] args) {
        ...
        for (int datasetId = 0; datasetId < datasetsD1.length; datasetId++) {
            ...
            final BestPipelineForD2 bf1d2 = new BestPipelineForD2(duplicatePropagation, profiles1, profiles2);
            bf1d2.applyWorkflow();
            bf1d2.printPerformance();
        }
    }
}
