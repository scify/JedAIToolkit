 ...
 * @author Georgios
 */
public class BestPipelineForD10 extends AbstractBestConfiguration {

    public BestPipelineForD10(AbstractDuplicatePropagation dp, List<EntityProfile> pf1, List<EntityProfile> pf2) {
        super(dp, pf1, pf2);
    }
    ...
    public void applyWorkflow() {
        ...
        bf.setNumberedGridConfiguration(22);
        blocks = bf.refineBlocks(blocks);

        final AbstractMetablocking mb = new BLAST();
        mb.setNumberedGridConfiguration(5);
        blocks = mb.refineBlocks(blocks);

        long time2 = System.currentTimeMillis();

        final ProfileMatcher pm = new ProfileMatcher(profiles1, profiles2,
                RepresentationModel.TOKEN_UNIGRAMS_TF_IDF, SimilarityMetric.ARCS_SIMILARITY);
        simPairs = pm.executeComparisons(blocks);

        umc = new UniqueMappingClustering();
        umc.setNumberedGridConfiguration(4);
    ...
    public static void main(String[] args) {
        ...
        for (int datasetId = 0; datasetId < datasetsD1.length; datasetId++) {
            ...
            GtSerializationReader gtReader = new GtSerializationReader(mainDir + groundtruthDirs[datasetId]);
            final AbstractDuplicatePropagation duplicatePropagation = new BilateralDuplicatePropagation(gtReader.getDuplicatePairs(null));
            System.out.println("Existing Duplicates\t:\t" + duplicatePropagation.getDuplicates().size());

            final BestPipelineForD10 bf1d10 = new BestPipelineForD10(duplicatePropagation, profiles1, profiles2);
            bf1d10.applyWorkflow();
            bf1d10.printPerformance();
        }
    }
}
