 ...
 * @author Georgios
 */
public class BestPipelineForD5 extends AbstractBestConfiguration {

    public BestPipelineForD5(AbstractDuplicatePropagation dp, List<EntityProfile> pf1, List<EntityProfile> pf2) {
        super(dp, pf1, pf2);
    }
    ...
    public void applyWorkflow() {
        ...
        bf.setNumberedGridConfiguration(36);
        blocks = bf.refineBlocks(blocks);

        final AbstractMetablocking mb = new CardinalityEdgePruning();
        mb.setNumberedGridConfiguration(2);
        blocks = mb.refineBlocks(blocks);

        long time2 = System.currentTimeMillis();

        final ProfileMatcher pm = new ProfileMatcher(profiles1, profiles2,
                RepresentationModel.CHARACTER_FOURGRAMS_TF_IDF, SimilarityMetric.ARCS_SIMILARITY);
        simPairs = pm.executeComparisons(blocks);

        umc = new UniqueMappingClustering();
        umc.setNumberedGridConfiguration(16);
    ...
    public static void main(String[] args) {
        ...
        for (int datasetId = 0; datasetId < datasetsD1.length; datasetId++) {
            ...
            final BestPipelineForD5 bf1d5 = new BestPipelineForD5(duplicatePropagation, profiles1, profiles2);
            bf1d5.applyWorkflow();
            bf1d5.printPerformance();
        }
    }
}
