 ...
 * @author Georgios
 */
public class BestPipelineForD4 extends AbstractBestConfiguration {

    public BestPipelineForD4(AbstractDuplicatePropagation dp, List<EntityProfile> pf1, List<EntityProfile> pf2) {
        super(dp, pf1, pf2);
    }
    ...
    public void applyWorkflow() {
        ...
        bf.setNumberedGridConfiguration(11);
        blocks = bf.refineBlocks(blocks);

        final AbstractMetablocking mb = new WeightedNodePruning();
        mb.setNumberedGridConfiguration(0);
        blocks = mb.refineBlocks(blocks);

        long time2 = System.currentTimeMillis();

        final ProfileMatcher pm = new ProfileMatcher(profiles1, profiles2,
                RepresentationModel.TOKEN_UNIGRAMS, SimilarityMetric.COSINE_SIMILARITY);
        simPairs = pm.executeComparisons(blocks);

        umc = new UniqueMappingClustering();
        umc.setNumberedGridConfiguration(9);
    ...
    public static void main(String[] args) {
        ...
        for (int datasetId = 0; datasetId < datasetsD1.length; datasetId++) {
            ...
            final BestPipelineForD4 bf1d4 = new BestPipelineForD4(duplicatePropagation, profiles1, profiles2);
            bf1d4.applyWorkflow();
            bf1d4.printPerformance();
        }
    }
}
