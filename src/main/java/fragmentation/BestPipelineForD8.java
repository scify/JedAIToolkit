 ...
 * @author Georgios
 */
public class BestPipelineForD8 extends AbstractBestConfiguration {

    public BestPipelineForD8(AbstractDuplicatePropagation dp, List<EntityProfile> pf1, List<EntityProfile> pf2) {
        super(dp, pf1, pf2);
    }
    ...
    public void applyWorkflow() {
        ...
        bf.setNumberedGridConfiguration(2);
        blocks = bf.refineBlocks(blocks);

        final AbstractMetablocking mb = new WeightedEdgePruning(WeightingScheme.CARDINALITY_NORM_CBS);
        ...
        final ProfileMatcher pm = new ProfileMatcher(profiles1, profiles2,
                RepresentationModel.CHARACTER_BIGRAMS_TF_IDF, SimilarityMetric.COSINE_SIMILARITY);
        simPairs = pm.executeComparisons(blocks);

        umc = new UniqueMappingClustering();
        umc.setNumberedGridConfiguration(12);
    ...
    public static void main(String[] args) {
        ...
        for (int datasetId = 7; datasetId < datasetsD1.length; datasetId++) {
            ...
            final BestPipelineForD8 bf1d8 = new BestPipelineForD8(duplicatePropagation, profiles1, profiles2);
            bf1d8.applyWorkflow();
            bf1d8.printPerformance();
        }
    }
}
